...
def check_and_copy_output_original():
    ...
    if os.path.exists("temp"):
        shutil.rmtree("temp")  # Ensure temp folder is completely cleared
    os.makedirs("temp", exist_ok=True)  # Recreate an empty temp folder
    shutil.copytree("_OUTPUT_ORIGINAL", "temp", dirs_exist_ok=True)
    return True
...
def convert_to_flac():
    for root, dirs, files in os.walk("temp"):
        for file in files:
            ...
            command = [
                "ffmpeg", "-i", file_path, "-c:a", "flac", "-metadata", f"artist={artist_name}",
